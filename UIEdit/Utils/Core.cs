...
namespace UIEdit.Utils
{
    public class Core
    {
        ...
        public static void RemoveAllPNGs()
        {
            if (MessageBox.Show($"Be aware, this is a beta feature, continue?", "Confirmation", MessageBoxButtons.YesNo) == DialogResult.No)
            {
                return;
            }
            MessageBox.Show($"Please wait, it may take some time...");
            var onDeletion = new HashSet<string>();
            int found = 0;
            foreach (var item in MainWindow.Instance.ProjectController.Files)
            {
                string res = ProjectController.Instance.GetSourceFileContent(item);
                var exceptionParse = LayoutController.Instance.Parse(res, ProjectController.Instance.SurfacesPath);
                if (exceptionParse == null)
                {
                    string lastFileName = string.Empty;
                    foreach (var filename in LayoutController.Dialog.ImagePictures.Select(x=>x.FileName))
                    {
                        if (filename is null || filename == lastFileName)
                        {
                            continue;
                        }
            ...
            if (found > 0)
            {
                var choice = MessageBox.Show($"All done! Found {found} duplicated files. Delete them?", "Confirmation", MessageBoxButtons.YesNo);
                if (choice == DialogResult.Yes)
                {
                    string msg = "Success! All files deleted!";
                    try
                    {
                        foreach (var del_item in onDeletion)
                        {
                            File.Delete(del_item);
                        }
                    }
                    catch (Exception e)
                    {
                        msg = $"An error occured while deleting files... Contact with dev\nError message: {e.Message}";
                    }
                    MessageBox.Show(msg);
                }
                else
                {
                    MessageBox.Show("Files remain still");
                }
            } else
            {
                MessageBox.Show("No duplicated files found!");
            }
        }
